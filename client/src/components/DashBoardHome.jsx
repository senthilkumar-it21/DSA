...
import { getAllAlbums, getAllArtist, getAllPlaylists, getAllSongs, getAllUsers } from "../api";
...
const DashBoardHome = () => {
  const [{ allUsers, allSongs, artists, allAlbums ,allPlaylists}, dispatch] =
    useStateValue();
  useEffect(() => {
    if (!allUsers) {
    ...
    if (!allPlaylists) {
      getAllAlbums().then((data) => {
        dispatch({ type: actionType.SET_ALL_PLAYLISTS, allPlaylists: data.data });
      });
    }
  ...
  return (
    <div className="w-full p-6 flex items-center justify-evenly flex-wrap">
      ...
      <DashboardCard icon={<GiMusicalNotes className="text-3xl text-textColor" />} name={"Playlist"} count={allPlaylists?.length > 0 ? allPlaylists?.length : 0} />
    </div>
  );
};
